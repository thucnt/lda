...
class LDAExample(sc: SparkContext, spark: SparkSession) {
  ...
  def run(params: Params): Unit = {
    ...
    if (ldaModel.isInstanceOf[DistributedLDAModel]) {
      ...
      println("---------------------------")
      val topicDistr = distLDAModel.topicDistributions.collect()
      for (doc <- topicDistr){
...
//
//    val df_mysql = sqlContext.read
//      .format("jdbc")
//      .option("url", "jdbc:mysql://localhost:3306/mas")
//      .option("driver", "com.mysql.jdbc.Driver")
//      .option("dbtable", "(select idPaper, title from paper LIMIT 100) as paper_content")
//      .option("user", "root")
//      .option("password", "thuc1980")
//      .load()
//
//    val initialrdd = df_mysql.rdd.map {
//      case Row(key: Long, value: String) =>
//        key -> value
//      case Row(key: Long) =>
//        key -> "Empty"
//    }.map(_._2)
    ...
    val initialrdd = db.Corpus.getPapers.rdd.map(_._2)
    initialrdd.cache()
...
object Main extends App {
  ...
  val conf = new SparkConf().setAppName(s"LDAExample").setMaster("local[4]").set("spark.executor.memory", "8g")
